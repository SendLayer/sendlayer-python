...
def test_create_webhook_validation(webhooks_client):
    ...
    with pytest.raises(SendLayerValidationError, match="Error: Invalid webhook URL"):
        webhooks_client.create("not-a-url", "open")
    
    # Test invalid event type
    with pytest.raises(SendLayerValidationError, match="Error: Invalid event name"):
        webhooks_client.create("https://example.com/webhook", "invalid")
